 ...
 *
 * Two shapes:
 *   --mode projects (default)  every sheet becomes its own ScrumFlow project (matches the tracker's
 *                              one-tab-per-project layout); each project gets its own sprints.
 *   --mode epics               one project, every sheet becomes an epic.
 *
 * Usage:
 *   node tools/tracker-to-scrumflow.mjs <tracker.xlsx> [--curation curation.json] [--out backup.json]
 *                                       [--mode projects|epics] [--key PRJ] [--name "Project name"] [--today 2026-09-17]
 *
 * The optional curation file lets you rename items, fix owners/status/ETA, assign groups (sheet → project/epic),
...
if (!input) {
  console.error('usage: node tools/tracker-to-scrumflow.mjs <tracker.xlsx> [--curation file] [--out file] [--mode projects|epics] [--key PRJ] [--name "Project"] [--today yyyy-mm-dd]')
  process.exit(1)
}
const curation = opt('curation') ? JSON.parse(fs.readFileSync(opt('curation'), 'utf8')) : {}
const MODE = opt('mode', curation.mode ?? 'projects')
const PROJECT_MODE = MODE === 'projects'
...
const singleKey = (opt('key', curation.project?.key ?? 'TRK')).toUpperCase()
const singleName = opt('name', curation.project?.name ?? path.basename(input, path.extname(input)))
...
function parseDate(raw, assumeYear = 2026) {
  ...
  const s = t(raw).replace(/[Oo](?=\d)/g, '0').replace(/\(.*?\)/g, '').trim()
  if (!s || /^tbd$/i.test(s)) return undefined
  let m = s.match(/^(\d{4})-(\d{2})-(\d{2})/)
  if (m) return `${m[1]}-${m[2]}-${m[3]}`
  m = s.match(/^(\d{1,2})-(\d{2})(\d{4})$/) // "18-082026"
  if (m) return `${m[3]}-${m[2]}-${pad(m[1])}`
...
const isoFromDay = (day, h) => iso(...day.split('-').map(Number), h)
...
function member(name) {
  ...
  if (!members.has(name)) {
    ...
    members.set(name, { id: uid('m'), name, role: cfg.role ?? 'dev', capacity: cfg.capacity ?? 8, color: COLORS[members.size % COLORS.length], active: true, createdAt: EPOCH })
  }
  return members.get(name)
}
for (const name of Object.keys(curation.members ?? {})) member(name)

/* ---------- groups: a group is a project (projects mode) or an epic inside one project (epics mode) ---------- */
...
const projects = []
const groups = new Map() // key → { key, project, epic?, labels }
const items = []
const comments = []
const itemByKey = new Map()
let rank = 0

function makeProject(key, name, description) {
  const p = { id: uid('p'), key, name, description, columns: DEFAULT_COLUMNS.map((c) => ({ ...c })), memberIds: [], nextNumber: 1, sprintLengthDays: 14, createdAt: EPOCH, updatedAt: NOW }
  projects.push(p)
  return p
}
const singleProject = PROJECT_MODE ? undefined : makeProject(singleKey, singleName, curation.project?.description ?? '')

function keyFromTitle(title) {
  return title.replace(/[^A-Za-z0-9 ]/g, ' ').split(/\s+/).filter(Boolean).map((w) => w[0]).join('').toUpperCase().slice(0, 6) || 'PRJ'
}
function groupFor(key) {
  ...
  let g
  if (PROJECT_MODE) {
    const project = makeProject((cfg.key ?? keyFromTitle(cfg.title)).toUpperCase(), cfg.title, cfg.description ?? '')
    g = { key, project, labels: cfg.labels ?? [] }
  } else {
    const epic = {
      id: uid('i'),
    ...
    items.push(epic)
    g = { key, project: singleProject, epic, labels: cfg.labels ?? [] }
  }
  groups.set(key, g)
  return g
}
for (const k of Object.keys(curation.epics ?? {})) groupFor(k)
...
for (const [sheet, aoa] of Object.entries(sheets)) {
  ...
  const cUpdate = (() => {
    const i = findCol(hdr, 'latest update', 'blocker')
    return i >= 0 ? i : findCol(hdr, 'update')
  })()
  const c = {
    ...
    update: cUpdate,
    update2: hdr.findIndex((h, i) => /^update$/i.test(h) && i !== cUpdate),
  ...
  const groupKey = isTickets ? curation.ticketsEpic ?? sheet : curation.sheetEpic?.[sheet] ?? sheet
  ...
  for (let r = hdrIdx + 1; r < aoa.length; r++) {
    ...
    if (!rawTitle) {
      if (sl) change(key, 'row', '(empty)', '(dropped)', 'empty row')
      continue
    }
    ...
    const priority = PRIORITY_MAP[t(row[c.priority]).toLowerCase()] ?? 'medium'
    ...
    const upd = parseUpdates(c.update >= 0 ? row[c.update] : '')
    const extra = c.update2 >= 0 ? t(row[c.update2]) : ''
    const remarks = c.remarks >= 0 ? t(row[c.remarks]) : ''
    const targetGroup = cur.epic ? groupFor(cur.epic) : group
    if (cur.epic && cur.epic !== groupKey) change(key, PROJECT_MODE ? 'project' : 'epic', groups.get(groupKey)?.project.name ?? groupKey, targetGroup.project.name, 'regrouped')
    const labels = new Set([...(PROJECT_MODE ? [] : targetGroup.labels), ...envLabels(cur.env ?? row[c.env]), ...coOwners, ...(cur.labels ?? [])])
    ...
    const createdAt = isoFromDay(first, 9)
    const updatedAt = isoFromDay(last, 18)
    ...
    const item = {
      id: uid('i'),
      projectId: targetGroup.project.id,
      number: 0,
    ...
    for (const e of upd.entries) comments.push({ id: uid('c'), itemId: item.id, authorId: assignee?.id, body: e.text, createdAt: isoFromDay(e.date, 12) })
  }
}

/* second pass: parents, dependencies */
for (const it of items) {
  if (it._parentKey) {
    const p = itemByKey.get(it._parentKey)
    if (p) {
      if (p.projectId !== it.projectId) {
        change(it._key, 'project', projects.find((x) => x.id === it.projectId)?.name, projects.find((x) => x.id === p.projectId)?.name, "moved into the parent item's project")
        it.projectId = p.projectId
      }
      it.parentId = p.id
      if (it.type === 'story') it.type = 'task'
      change(it._key, 'parent', PROJECT_MODE ? '(none)' : '(epic)', it._parentKey, 'nested under related item')
...
/* numbering per project, in rank order; project membership = people with items there + curated core members */
for (const p of projects) {
  let n = 1
  for (const it of items.filter((i) => i.projectId === p.id).sort((a, b) => a.rank - b.rank)) it.number = n++
  p.nextNumber = n
  const ids = new Set(items.filter((i) => i.projectId === p.id).map((i) => i.assigneeId).filter(Boolean))
  p.memberIds = [...members.values()].filter((m) => !PROJECT_MODE || ids.has(m.id) || curation.members?.[m.name]?.core).map((m) => m.id)
}

/* ---------- Project_Plan → go-live milestone on the group ---------- */
  ...
  for (const row of plan.slice(1)) {
    ...
    if (/go-live|golive/i.test(t(row[cp.phase])) || /^yes$/i.test(t(row[cp.milestone]))) {
      ...
      if (g && d) {
        ...
        } else g.project.description += `${g.project.description ? '\n\n' : ''}Go-live milestone (Project_Plan): ${d}.`
      }
    }
  }
}
...
const sprints = []
for (const p of projects) {
  const mine = items.filter((i) => i.projectId === p.id && i.type !== 'epic')
  const h = curation.historySprint
  if (h && mine.some((i) => i.status === 'done')) {
    const s = { id: uid('s'), projectId: p.id, name: h.name, goal: h.goal ?? '', startDate: h.start, endDate: h.end, status: 'completed', capacity: 0, committedPoints: 0, completedPoints: 0, createdAt: EPOCH, updatedAt: NOW }
    sprints.push(s)
    for (const it of mine.filter((i) => i.status === 'done')) {
      it.sprintId = s.id
      it.sprintAddedAt = it.createdAt
    }
  }
  const a = curation.sprint
  if (a) {
    const inFlight = mine.filter((it) => it.status !== 'done' && (it.status === 'inprogress' || it.status === 'blocked' || it.labels.includes('recurring') || (it.dueDate && it.dueDate <= a.end)))
    if (inFlight.length) {
      const s = { id: uid('s'), projectId: p.id, name: a.name, goal: a.goal ?? '', startDate: a.start, endDate: a.end, status: 'active', capacity: a.capacity ?? 0, committedPoints: 0, createdAt: NOW, updatedAt: NOW }
      sprints.push(s)
      for (const it of inFlight) {
        it.sprintId = s.id
        it.sprintAddedAt = isoFromDay(a.start, 9)
...
const snapshot = {
  ...
  sprints,
  comments,
  activities: items.map((i) => ({ id: uid('a'), projectId: i.projectId, itemId: i.id, actorId: i.assigneeId, kind: 'created', at: i.createdAt })),
  standups: [],
  retroNotes: [],
  retroActions: [],
  notifications: [],
  settings: { id: 'app', orgName: opt('org', curation.orgName ?? 'My Organization'), theme: 'system', currentProjectId: projects[0]?.id, currentMemberId: memberList[0]?.id, seeded: true },
}
fs.writeFileSync(outFile, JSON.stringify(snapshot, null, 1))
fs.writeFileSync(outFile.replace(/\.json$/, '') + '.changes.json', JSON.stringify({ mode: MODE, warnings, changes }, null, 1))

/* ---------- report ---------- */
console.log(`Wrote ${outFile}  (mode: ${MODE})`)
console.log(`  members ${memberList.length} | projects ${projects.length} | items ${items.filter((i) => i.type !== 'epic').length} | comments ${comments.length} | sprints ${sprints.length}`)
for (const p of projects) {
  const mine = items.filter((i) => i.projectId === p.id && i.type !== 'epic')
  const by = mine.reduce((a, i) => ((a[i.status] = (a[i.status] ?? 0) + 1), a), {})
  console.log(`  ${p.key.padEnd(7)} ${p.name.padEnd(42)} ${String(mine.length).padStart(3)} items  ${JSON.stringify(by)}`)
}
